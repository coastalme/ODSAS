...
int CDelineation::nLocateCliffTop(void)
{
   ...
   {
      int const nNumProfiles = static_cast<int>(m_VCoast[nCoast].nGetNumProfiles());
     ...
     // Get the nCoastPoint of the profile indicating the  
	 ...
	 // Get the length of the profile (in cells) and the index of the coast point at which this profile starts
         int const  nProfSize = static_cast<int>(pProfile->nGetNumCellsInProfile());
	...
	// cout << "nProfSize, m_bTooShort: " << nProfSize << ", " << pProfile->bTooShort() << endl;
	 ...
	 // Calculate the length of the profile in external CRS units
         int const nSegments = static_cast<int>(pProfile->nGetProfileSize())-1;
    ...
    //     {
     //    int nRet = nSaveProfile(nCoastPoint, nCoast, nProfSize, &dVProfileZ, pProfile->pPtiVGetCellsInProfile());
     //    if (nRet != RTN_OK)
    //       return nRet;
     //    }
