...
#include "ogrsf_frmts.h"            // For OGR vector file reading
#include "ogr_api.h"                // For OGR API functions
using std::ifstream;
...
{
   m_strODSASIni = m_strODSASDir;
   m_strODSASIni.append(ODSAS_INI);

   // The .ini file is assumed to be in the ODSAS executable's directory
   string strFilePathName(m_strODSASIni);
   ...
   while (InStream.getline(szRec, BUFSIZE, '\n'))
   {
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
         ...
         case 1:
            ...
            else
            {
               ...
               if (m_strDataPathName.empty())
               {
                  ...
                  {
                     // It has a relative path, so prepend the ODSAS dir
                     m_strDataPathName = m_strODSASDir;
         ...
         case 2:
            // Path for ODSAS output
            if (strRH.empty())
               strErr = "path for ODSAS output";
            else
            {
               // Check for trailing slash on ODSAS output directory name (is vital)
               ...
               {
                  // It is a relative path, so prepend the ODSAS dir
                  m_strOutPath = m_strODSASDir;
   ...
   while (InStream.getline(szRec, BUFSIZE, '\n'))
   {
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
        ...
        string strTmp;
         ...
         // ---------------------------------------------- Run Information -----------------------------------------------------
         case 1:
            //  Profile Type (0=Land, 1=Sea and 2=Both)
              m_nTypeProfile = atoi(strRH.c_str());
              if ((m_nTypeProfile < 0) || (m_nTypeProfile > 2))
               strErr = "value of Profile Type must be 0=Land, 1=Sea and 2=Both";
            break;

         case 2:
            // Text output file names, don't change case
            if (strRH.empty())
               strErr = "missing output file name";
            else
            {
               m_strRunName = strRH;
         ...
         case 3:
            //  DTM of GRID filename (can't be blank)
            if (! strRH.empty())
            {
#ifdef _WIN32
               // For Windows, make sure has backslashes, not Unix-style slashes
         ...
         case 8:
            // The vector polyline filename with the historical shorelines (can't be blank) (any GDAL format, e.g. SHP, GPKG,...)
            if (! strRH.empty())
            {
               #ifdef _WIN32
               // For Windows, make sure has backslashes, not Unix-style slashes
               ...
               {
                  // It has an absolute path, so use it 'as is'
                  m_strMulticoastlineFile = strRH;
               }
               else
               {
                  // It has a relative path, so prepend the ODSAS dir
                  m_strMulticoastlineFile = m_strODSASDir;
                  m_strMulticoastlineFile.append(strRH);
               }
            }
            break;

	       case 9:
            // The CSV file with the ID, date & time, uncertainty of the historical shorelines in filename defined in case 8
            if (! strRH.empty())
            {
               #ifdef _WIN32
               // For Windows, make sure has backslashes, not Unix-style slashes
               ...
               {
                  // It has an absolute path, so use it 'as is'
                  m_strMulticoastDatesFile = strRH;
               }
               else
               {
                  // It has a relative path, so prepend the ODSAS dir
                  m_strMulticoastDatesFile = m_strODSASDir;
                  m_strMulticoastDatesFile.append(strRH);
               }
            }
            break;
...
////////
         ...
         case 10:
            // Filename of the point vector file with the user defined baseline (optional, can be blank and if empty it will be calculated from DTM)
            if (! strRH.empty())
            {
            #ifdef _WIN32
               // For Windows, make sure has backslashes, not Unix-style slashes
               ...
               {
                  // It has a relative path, so prepend the ODSAS dir
                  m_strInitialCoastlineFile = m_strODSASDir;
         ...
         case 11:
         {
            if (strRH.empty())
            {
               // If no manual value provided, try to auto-detect from coastline file
               if (!m_strInitialCoastlineFile.empty())
               {
                  // Try to read SEAis field from the coastline vector file
                  GDALAllRegister();
                  
                  GDALDataset* poDataset = (GDALDataset*)GDALOpenEx(m_strInitialCoastlineFile.c_str(), 
                  ...
                  if (poDataset != NULL)
                  {
                     ...
                     if (poLayer != NULL)
                     {
                        // Check if SEAis field exists
                        OGRFeatureDefn* poFDefn = poLayer->GetLayerDefn();
                        int nSEAisField = poFDefn->GetFieldIndex("SEAis");
                        
                        if (nSEAisField >= 0)
                        {
                           // SEAis field exists, read the first feature
                           poLayer->ResetReading();
                           OGRFeature* poFeature = poLayer->GetNextFeature();
                           
                           if (poFeature != NULL)
                           {
                              const char* pszSEAis = poFeature->GetFieldAsString(nSEAisField);
                              if (pszSEAis != NULL)
                              {
                                 string strSEAis = string(pszSEAis);
                                 // Convert to lowercase for comparison
                                 std::transform(strSEAis.begin(), strSEAis.end(), strSEAis.begin(), ::tolower);
                                 
                                 if (strSEAis == "right")
                                 {
                                    m_nCoastSeaHandiness = 0;
                                    cout << "Auto-detected sea handiness from SEAis field: " << m_nCoastSeaHandiness << " (right)" << endl;
                                 }
                                 else if (strSEAis == "left")
                                 {
                                    m_nCoastSeaHandiness = 1;
                                    cout << "Auto-detected sea handiness from SEAis field: " << m_nCoastSeaHandiness << " (left)" << endl;
                                 }
                                 else
                                 {
                                    strErr = "invalid SEAis field value (must be 'right' or 'left')";
                                 }
                              }
                              else
                              {
                                 strErr = "SEAis field is empty in coastline file";
                              }
                              OGRFeature::DestroyFeature(poFeature);
                           }
                           else
                           {
                              strErr = "no features found in coastline file";
                           }
                        }
                        else
                        {
                           strErr = "SEAis field not found in coastline file and no manual value provided";
                        }
                     }
                     else
                     {
                        strErr = "cannot access layer in coastline file";
                     }
                     GDALClose(poDataset);
                  }
                  else
                  {
                     strErr = "cannot open coastline file for SEAis field reading";
                  }
               }
               else
               {
                  strErr = "value of Coast Sea Handiness for user defined coastline (must be 0 or 1) or provide SEAis field in coastline file";
               }
            }
            else
            {
               ...
               cout << "Using user-specified sea handiness: " << m_nCoastSeaHandiness << endl;
            }

            if ((m_nCoastSeaHandiness < 0) || (m_nCoastSeaHandiness > 1))
               strErr = "value of Coast Sea Handiness for user defined coastline (must be 0 or 1)";
            break;
         }
           
         case 12:
           // on which edge of the DTM the start of the user defined coastline is? [N = 1, E = 2, S = 3, W = 4]
	       m_nStartEdgeUserCoastline = atoi(strRH.c_str());
            if ((m_nStartEdgeUserCoastline < 0) || (m_nStartEdgeUserCoastline > 4))
               strErr = "value of Coast start Edge for user defined coastline (must be >0 & <= 4)";
            break;
        
         case 13:
           // on which edge of the DTM the end of the user defined coastline is? [N = 1, E = 2, S = 3, W = 4]
	       m_nEndEdgeUserCoastline = atoi(strRH.c_str());
           if ((m_nEndEdgeUserCoastline < 0) || (m_nEndEdgeUserCoastline > 4))
               strErr = "value of Coast end Edge for user defined coastline (must be >0 & <= 4)";
            break;
         ...
         case 20:
            ...
            //CTdistance profile in option BOTH will be a half
             if (m_nTypeProfile == 2)
            {
               m_dCoastNormalLength =m_dCoastNormalLength/2;
            }
            break;
         case 21:
