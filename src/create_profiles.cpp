...
int CDelineation::nCreateAllProfiles(void)
{
   ...
   if (m_nTypeProfile == 0)
   std::cout << READTYPEPROFILE + "LAND" << std::endl;
   if (m_nTypeProfile == 1)
   std::cout << READTYPEPROFILE +  "SEA" << std::endl;
   if (m_nTypeProfile == 2)
   std::cout << READTYPEPROFILE + "SEA and LAND" << std::endl;
   
   for (unsigned int nCoast = 0; nCoast < static_cast<unsigned int>(m_VCoast.size()); nCoast++)
   {
   int nProfile = -1;
   size_t nCoastSize = m_VCoast[nCoast].nGetCoastlineSize();
   ...
   // int nCoastPoint; // Removed: unreferenced local variable
      for (size_t nCoastPoint = 0; nCoastPoint < nCoastSize; nCoastPoint++)
      {
         double dCurvature = m_VCoast[nCoast].dGetCurvature(static_cast<int>(nCoastPoint));
         prVCurvature.push_back(make_pair(static_cast<int>(nCoastPoint), dCurvature));
         dVCurvature.push_back(dCurvature);
      }
     // std::cout << "Num. Coastpoint initial size: " <<  nCoastPoint << std::endl;
      ...
      while (true)
      {
         ...
         //CT int profileSize = m_VCoast[nCoast].getHasProfileSize();
         //std::cout << "Vector hasprofile size: " <<  profileSize << std::endl;
...
int CDelineation::nCreateProfile(int const nCoast, int const nProfileStartPoint, int& nProfile)
{
   ...
   if (nGetCoastNormalEndPoint(nCoast, nProfileStartPoint, static_cast<int>(nCoastSize), &PtStart, m_dCoastNormalLength, &PtSeaEnd, &PtLandEnd) != RTN_OK)
...
//CT if
   if (m_nTypeProfile == 0) {
      // Action for Profile Type 0 (Land)
      VNormal.push_back(PtStart);
      VNormal.push_back(PtSeaEnd);     // to draw the profiles LANDWARDS!!!
   // cout << "Profile Type is Land" << endl;
   }
   else if (m_nTypeProfile == 1) {
      // Action for Profile Type 1 (Sea)
      // cout << "Profile Type is Sea" << endl;
      VNormal.push_back(PtStart);
      VNormal.push_back(PtLandEnd);  // to draw the profiles SEAWARDS!!!! 
   }
   else if (m_nTypeProfile == 2)
   {
      // Action for Profile Type 2 (Both), from SEA to LAND, 3 points this time
      // cout << "Profile Type is Both" << endl;

      VNormal.push_back(PtLandEnd);
      // Calculate the direction vector
      double dirX = PtLandEnd.dGetX() - PtStart.dGetX();
      double dirY = PtLandEnd.dGetY() - PtStart.dGetY();

      // Calculate the length of the original segment
      // double originalLength = m_dCoastNormalLength;

      // Normalize the direction vector
      double unitDirX = dirX / m_dCoastNormalLength;
      double unitDirY = dirY / m_dCoastNormalLength;

      // Calculate the extended point
      C2DPoint PtExtended(
          PtStart.dGetX() - m_dCoastNormalLength * unitDirX,
          PtStart.dGetY() - m_dCoastNormalLength * unitDirY);

      VNormal.push_back(PtExtended);

      // std::cout << "PtLand coordinates: (" << PtLandEnd.dGetX() << ", " << PtLandEnd.dGetY() << ")" << std::endl;
      // std::cout << "PtExtended coordinates: (" << PtExtended.dGetX() << ", " << PtExtended.dGetY() << ")" << std::endl;
   }
...
int CDelineation::nGetCoastNormalEndPoint(int const nCoast, int const nStartCoastPoint, int const nCoastSize, C2DPoint* const pPtStart, double const dLineLength, C2DPoint* pPtSeaEnd, C2DPoint* pPtLandEnd)
{
   ...
   else
   {
      ...
      // Now calculate the equation of the straight line which is perpendicular to this linking line: y = Ax + B
      double dAPerp = -1 / dA;
      double dBPerp = pPtStart->dGetY() - (dAPerp * pPtStart->dGetX());
      
      // Calculate the end points using x = x0 +/- d/sqrt(1+A^2) and the line equation
      dXEnd1 = pPtStart->dGetX() + dLineLength/sqrt(1+dAPerp*dAPerp);
      dYEnd1 = dAPerp*dXEnd1 + dBPerp;
      dXEnd2 = pPtStart->dGetX() - dLineLength/sqrt(1+dAPerp*dAPerp);
      dYEnd2 = dAPerp*dXEnd2 + dBPerp;
      
      /* OLD code: problem discriminant < 0 
      // The linking line runs neither W-E nor N-S so we have to work a bit harder to find the end-point of the coastline-normal profile
      double dA = dYDiff / dXDiff;
...
int CDelineation::nModifyAllIntersectingProfiles(void)
{
   ...
   {
   size_t nNumProfiles = m_VCoast[nCoast].nGetNumProfiles();

      // Go along the coast, looking at profiles which are increasingly distant from the first profile
   size_t nMaxDist = nNumProfiles / 2;       // Arbitrary
      for (size_t nDist = 1; nDist < nMaxDist; nDist++)
      {
         // Do once for every profile
         for (size_t nFirst = 0; nFirst < nNumProfiles; nFirst++)
         {
            // In coastline curvature sequence
            int nFirstProfile = m_VCoast[nCoast].nGetProfileAtAlongCoastlinePosition(static_cast<int>(nFirst));
            if (nFirstProfile < 0)
               // Not found
...
//            LogStream << m_ulTimestep << ": nFirst = " << nFirst << " nFirstProfile = " << nFirstProfile << endl;
            ...
            for (int nDirection = DIRECTION_DOWNCOAST; nDirection <= DIRECTION_UPCOAST; nDirection++)
            {
               ...
               if (nDirection == DIRECTION_DOWNCOAST)
                  nSecond = static_cast<int>(nFirst + nDist);
               else
                  nSecond = static_cast<int>(nFirst - nDist);

               if ((nSecond < 0) || (nSecond > static_cast<int>(nNumProfiles-1)))
...
//                      LogStream << m_ulTimestep << ": ^^^^ profiles " << nFirstProfile << " and " << nSecondProfile << " intersect, but point {" << dIntersectX << ", " << dIntersectY << "} is already present in profile " << nSecondProfile << " as point " << nPoint << endl;
   ...
   //                   assert(nProf2LineSeg < nSecondProfileLineSegments);
                     ...
                     if ((nProf1LineSeg == static_cast<int>(nFirstProfileLineSegments-1)) && (nProf2LineSeg == static_cast<int>(nSecondProfileLineSegments-1)))
                     {
                        ...
                        MergeProfilesAtFinalLineSegments(nCoast, nFirstProfile, nSecondProfile, static_cast<int>(nFirstProfileLineSegments), static_cast<int>(nSecondProfileLineSegments), dIntersectX, dIntersectY, dAvgEndX, dAvgEndY);
...
bool CDelineation::bCheckForIntersection(CProfile* const pVProfile1, CProfile* const pVProfile2, int& nProfile1LineSegment, int& nProfile2LineSegment, double& dXIntersect, double& dYIntersect, double& dXAvgEnd, double& dYAvgEnd)
{
   ...
   int
      nProfile1NumSegments = static_cast<int>(pVProfile1->nGetNumLineSegments()),
      nProfile2NumSegments = static_cast<int>(pVProfile2->nGetNumLineSegments());
...
int CDelineation::nPutAllProfilesOntoGrid(void)
{
   ...
   for (int nCoast = 0; nCoast < static_cast<int>(m_VCoast.size()); nCoast++)
   {
      ...
      int nProfiles = static_cast<int>(m_VCoast[nCoast].nGetNumProfiles());
      if (nProfiles == 0)
      {
      ...
      {  
         ...
         size_t nPoints = pProfile->nGetProfileSize();
         if (nPoints < 2)
         {
         ...
         RasterizeProfile(nCoast, nProfile, &VCellsToMark, &bVShared, bTruncated, bHitCoast, bHitAnotherProfile);
...
void CDelineation::RasterizeProfile(int const nCoast, int const nProfile, vector<C2DIPoint>* pVIPointsOut, vector<bool>* pbVShared, bool& bTooShort, bool& bTruncated, bool& bHitAnotherProfile) // AP eliminated unused  bool& bHitCoast
{
   CProfile* const pProfile = m_VCoast[nCoast].pGetProfile(nProfile);
   ...
   int
      nSeg = 0,
      nSegments = static_cast<int>(pProfile->nGetNumLineSegments()),
      nProfiles = static_cast<int>(m_VCoast[nCoast].nGetNumProfiles());    // TODO this is a bodge, needed if we hit a profile which belongs to a different coast object
   ...
   for (nSeg = 0; nSeg < nSegments; nSeg++)
   {
      ...
      //CT int n_HitCoasTTimes = 0; //CT Profile to both sides need to control the times that they hit to coastline
      ...
      for (int m = 0; m <= static_cast<int>(dRound(dLength)); m++)
      {
         ...
         if ((! pProfile->bStartOfCoast()) && (! pProfile->bEndOfCoast()))
         {
            ...
            //CT Both side profile can hit the coast one time, so the truncate is valid only for the second time
            //if ((nSeg == 0) && (m > 1))
            //{
               //CT NO TRUNCATE, section desactivated
              ...
              // {   
               ...
               // We've hit a coastline so set a switch and mark the profile and truncate 
                ...
                //  break;
                //  }
                //else if (m_nTypeProfile == 2)  //CT
                //  {
                //  n_HitCoasTTimes += 1;     
                //  cout << "Value of hitCoastTimes:" << n_HitCoasTTimes << endl; 
                //  if (n_HitCoasTTimes == 3) //CT the idea is to make a break in a second hit, but it doesn't work
                //     {
                //     bHitCoast = true;
                //     pProfile->SetHitCoast(true);                
                //     LogStream << m_ulTimestep << ": profile " << nProfile << " HIT COAST twice, last at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}" << endl;
                //     bTruncated = true;
                //     pProfile->SetTruncated(true);
                //     cout << "Profile truncate after two/three hits " << n_HitCoasTTimes << endl;
                //     break;
                //     }
                //  } 
...
//                }
   ...
   if (bTruncated)
   {
      ...
      int
         ...
         nLastX = pVIPointsOut->at(static_cast<size_t>(nOutSize-1)).nGetX(),
         nLastY = pVIPointsOut->at(static_cast<size_t>(nOutSize-1)).nGetY();
...
void CDelineation::MergeProfilesAtFinalLineSegments(int const nCoast, int const nFirstProfile, int const nSecondProfile, int const nFirstProfileLineSegments, int const nSecondProfileLineSegments, double const dIntersectX, double const dIntersectY, double const dAvgEndX, double const dAvgEndY)
{
   ...
   for (int n = 0; n < static_cast<int>(pFirstProfile->pprVGetCoincidentProfilesForLineSegment(nFirstProfileLineSegments-1)->size()); n++)
   {
      pair<int, int> prTmp;
   ...
   for (int n = 0; n < static_cast<int>(pSecondProfile->pprVGetCoincidentProfilesForLineSegment(nSecondProfileLineSegments-1)->size()); n++)
   {
      pair<int, int> prTmp;
...
//   LogStream << m_ulTimestep << ": END-SEGMENT INTERSECTION between profiles {" << nFirstProfile << "} and {" << nSecondProfile << "} at line segment " << nFirstProfileLineSegments-1 << "/" << nFirstProfileLineSegments-1 << ", and line segment " << nSecondProfileLineSegments-1 << "/" << nSecondProfileLineSegments-1 << ", respectively. Both truncated at [" << dIntersectX << ", " << dIntersectY << "] then profiles {" << nFirstProfile << "} and {" << nSecondProfile << "} extended to [" << dAvgEndX << ", " << dAvgEndY << "]" << endl;
   ...
   size_t nProfileLength = pThisProfile->nGetProfileSize();
   ...
   pThisProfile->SetPointInProfile(static_cast<int>(nProfileLength-1), dIntersectX, dIntersectY);
   }
...
//       LogStream << "\tFor profile {" << prVCoincidentProfiles[nn].first << "} line segment [" << nLineSegAfterIntersect[nn] << "] is immediately after the intersection point" << endl;
   ...
   size_t nNumLineSegs = pThisProfile->nGetNumLineSegments();

      // Do for all line segments seaward of the point of intersection
   for (int nLineSeg = nLineSegAfterIntersect[nn], nIncr = 0; nLineSeg < static_cast<int>(nNumLineSegs); nLineSeg++, nIncr++)
      {
      ...
      for (int nSeg = 0; nSeg < pThisProfile->nGetNumLineSegments(); nSeg++)
      {
         for (int nCoinc = 0; nCoinc < pThisProfile->nGetNumCoincidentProfilesInLineSegment(nSeg); nCoinc++)
         {
            ...
            else
            {
               ...
               int nNewProfsLineSeg = static_cast<int>(nVProfsLineSeg[nPos]);
               nNewProfsLineSeg++;
