...
void CDelineation::DoCoastCurvature(int const nCoast, int const nHandedness)
{
   ...
   int nAvgInterval = 9;                // Arbitrary, used at the coastline ends
   double
      dAvgKappaBegin = 0,
      dAvgKappaEnd = 0;
   ...
   for (size_t nThis = (nCoastSize - m_nCoastCurvatureInterval); nThis < nCoastSize; nThis++)
      m_VCoast[nCoast].SetCurvature(static_cast<int>(nThis), dAvgKappaEnd);
}
