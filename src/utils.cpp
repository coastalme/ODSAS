...
int CDelineation::nHandleCommandLineParams(int nArg, char* pcArgv[])
{
   for (int i = 1; i < nArg; i++)
   {
      ...
      {
         // User wants information about ODSAS
         cout << ABOUT << endl;
         cout << THANKS << endl;
...
/*==============================================================================================================================
 ...
 Finds the folder (directory) in which the ODSAS executable is located
...
bool CDelineation::bFindExeDir(char* pcArg)
{
   ...
   if (0 != GetModuleFileName(NULL, szBuf, BUFSIZE))
      ...
      strTmp = pcArg;
#else
//   char* pResult = getcwd(szBuf, BUFSIZE);          // Used to use this, but what if cwd is not the same as the ODSAS dir?
   ...
   m_strODSASDir = strTmp.substr(0, nPos+1);            // Note that this must be terminated with a backslash
...
void CDelineation::AnnounceLicence(void)
{
   ...
   // Note endl not needed, ctime outputs a trailing <cr>
#ifdef _WIN32
   char timeBuffer[26];
   ctime_s(timeBuffer, sizeof(timeBuffer), &m_tSysStartTime);
   cout << STARTNOTICE << strGetComputerName() << " on " << timeBuffer;
#else
...
/*==============================================================================================================================
 ...
 Tell the user that we are now reading the multi Coastline

==============================================================================================================================*/
 void CDelineation::AnnounceReadUserMCoastLine(void)
 {
#ifdef _WIN32
   cout << READVECTORFILES << pstrChangeToForwardSlash(&m_strMulticoastlineFile) << endl;
#else
   cout << READVECTORFILES << m_strMulticoastlineFile << endl;
#endif
 }
...
==============================================================================================================================*/
 ...
 {
#ifdef _WIN32
   cout << READVECTORFILES << pstrChangeToForwardSlash(&m_strInitialCoastlineFile) << endl;
#else
...
string CDelineation::strGetComputerName(void)
{
   ...
   size_t len = 0;
   if (_dupenv_s(&computerName, &len, "COMPUTERNAME") == 0 && computerName != nullptr)
   {
...
#ifdef _WIN32
   // Display Windows version info - using modern approach to avoid deprecated GetVersionEx
   OutStream << "Running under                                \t: Windows";
   
   // Try to get more specific version info using registry or other means if needed
   // For now, just show "Windows" to avoid deprecated API warnings
   // Note: GetVersionEx is deprecated and can return incorrect info on newer Windows versions
   
#pragma warning(push)
#pragma warning(disable: 4996)  // Temporarily disable deprecation warning
   ...
   if (! (bOsVersionInfoEx = GetVersionEx((OSVERSIONINFO *) &osvi)))
   {
      ...
      {
         // That didn't work either, use generic Windows
         OutStream << " (version unknown)" << endl;
         return;
      }
   }
#pragma warning(pop)  // Re-enable deprecation warning
...
void CDelineation::DoDelineationEnd(int const nRtn)
{
   ...
   case (RTN_OK):
      // normal ending
      cout << RUNENDNOTICE << __DATE__ << " @ " << __TIME__ << endl;
      break;
...
#ifdef _WIN32
      char timeBuffer1[26];
      ctime_s(timeBuffer1, sizeof(timeBuffer1), &m_tSysEndTime);
      cerr << ERRORNOTICE << nRtn << " (" << strGetErrorText(nRtn) << ") on " << timeBuffer1;
#else
...
#ifdef _WIN32
         char timeBuffer2[26];
         ctime_s(timeBuffer2, sizeof(timeBuffer2), &m_tSysEndTime);
         LogStream << ERR << strGetErrorText(nRtn) << " (error code " << nRtn << ") on " << timeBuffer2;
#else
...
#ifdef _WIN32
         char timeBuffer3[26];
         ctime_s(timeBuffer3, sizeof(timeBuffer3), &m_tSysEndTime);
         OutStream << ERR << strGetErrorText(nRtn) << " (error code " << nRtn << ") on " << timeBuffer3;
#else
